                          ...
                          struct secure_conn *nw_security_opts)
{
    ...
    //If TLS is required
    if ((nwconn_opts & DEV_NETCONN_OPT_SEC) != 0) {
...
#if ((SL_MAJOR_VERSION_NUM >= 2) || ((SL_MAJOR_VERSION_NUM == 1) && (SL_VERSION_NUM >= 1)))
/* If Gen2 or SDK 1.20 (Host Driver 1.0.1.x) */
        if (NULL != nw_security_opts->remote_dn_str) {
    		Status = sl_SetSockOpt(MqttSocketFd, SL_SOL_SOCKET, SO_SECURE_DOMAIN_NAME_VERIFICATION,
                                       nw_security_opts->remote_dn_str,
                                       strlen(nw_security_opts->remote_dn_str));
			if (Status < 0) {
				sl_Close(MqttSocketFd);
				return (Status);
			}
...
#if (SL_MAJOR_VERSION_NUM >= 2)
        if (false == nw_security_opts->dev_listed_ca) {
        	_u32 dummyVal;
                Status = sl_SetSockOpt(MqttSocketFd, SL_SOL_SOCKET, SL_SO_SECURE_DISABLE_CERTIFICATE_STORE,
    ...
    // If no TLS required
        ...
        if ((nwconn_opts & DEV_NETCONN_OPT_UDP) != 0) {
...
static _u32 svr_addr_NB_order_IPV4(char *svr_addr_str)
{
    ...
    char *svr_addr_cpy = MQTT_MALLOC(svr_addr_size + 1); //1 for null
               ...
               const struct secure_conn *nw_security)
{
    ...
    if ((nwconn_opts & DEV_NETCONN_OPT_UDP) != 0) {
        ...
        {
            if ((nwconn_opts & DEV_NETCONN_OPT_IP6) != 0) {
				// server address isan IPV6 address string  
			  ...
			  void *ctx)
{
    ...
    Status = sl_SetSockOpt(MqttSocketFd, SL_SOL_SOCKET, SL_SO_RCVTIMEO, &timeVal,
            sizeof(timeVal));
               ...
               const _u8 *dest_ip, _u32 ip_len)
{
    ...
    //get destination ip
#if 0
	//assuming a IPV4 address is passed in dot notation.
    uiDestIp = svr_addr_NB_order_IPV4((_u8*)dest_ip); 
    if( 0 == uiDestIp )
    {
